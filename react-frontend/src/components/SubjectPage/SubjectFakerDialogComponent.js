...
import subjectFakerFactory from "./subjectFakerFactory";
...
const SubjectFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = subjectFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("subject").create(elem));
...
export default connect(mapState, mapDispatch)(SubjectFakerDialogComponent);
