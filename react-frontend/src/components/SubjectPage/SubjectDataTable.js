...
const SubjectDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.subjectID}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.subjectName}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "subject"});
...
<Column field="subjectID" header="SubjectID" body={pTemplate0} filter={selectedFilterFields.includes("subjectID")} hidden={selectedHideFields?.includes("subjectID")}  sortable style={{ minWidth: "8rem" }} />
<Column field="subjectName" header="Subject Name" body={pTemplate1} filter={selectedFilterFields.includes("subjectName")} hidden={selectedHideFields?.includes("subjectName")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Subject Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Subject" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
